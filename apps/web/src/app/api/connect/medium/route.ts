...
}, (t) => [index("pa_medium_idx").on(t.userId, t.platform)]);
...
export async function POST(req: NextRequest) {
  ...
  const rssRes = await fetch(`https://medium.com/feed/@${handle}`, {
    headers: { "User-Agent": "DevCreator/1.0" },
  });
  if (!rssRes.ok) {
    return NextResponse.json({ error: "Medium profile not found" }, { status: 404 });
  }
  ...
  const [existing] = await db.select().from(platformAccounts).where(
    and(eq(platformAccounts.userId, user.id), eq(platformAccounts.platform, "medium"))
  );

  if (existing) {
    await db.update(platformAccounts)
      .set({ handle, connectedAt: new Date() })
      .where(and(eq(platformAccounts.userId, user.id), eq(platformAccounts.platform, "medium")));
  } else {
    await db.insert(platformAccounts).values({
      userId: user.id, platform: "medium", handle, accessToken: "rss",
    });
  }
